#!/usr/bin/env python3
"""Fixed early-answer intervention replication on a second frozen checkpoint."""
import argparse
import csv
import json
...
def validate_manifest(rows):
    ...
    if not all(repetitions):
        raise ValueError('Explicit nonempty repetition identities required')
    keys = [(r['actor'], r['statement'], r['repetition'], r['emotion']) for r in rows]
    expected = {(a, s, r, e) for a in actors for s in statements for r in repetitions for e in ('happy', 'sad')}
    if len(keys) != len(set(keys)) or set(keys) != expected:
        raise ValueError('Expected complete rectangular actor x statement x repetition x happy/sad coverage')
    return actors, statements


def within_folds(rows):
    """All repetitions stay together in each actor/statement test fold."""
...
def plot(summary, path, dataset):
    ...
    fig.suptitle(f'{MODEL} · {dataset}: fixed blocks 1–6')
    fig.tight_layout()
...
def analyze(args):
    ...
    n_repetitions = len({r['repetition'] for r in rows})
    ...
    for endpoint in ENDPOINTS:
        ...
        for cidx, condition in enumerate(CONDITIONS):
            ...
            for fold, train, test in splits:
                ...
                predictions.extend(dict(endpoint=endpoint, condition=condition, fold=fold, **{k: rows[i][k] for k in ('sample_id', 'actor', 'statement', 'repetition', 'emotion')}, score=float(value), prediction='happy' if value > 0 else 'sad') for i, value in zip(test, values))
    ...
    info = dict(model=MODEL, dataset=args.dataset, n_repetitions=n_repetitions, n_samples=len(rows), n_actors=len(actor_names), n_statements=len(statement_names), actors=actor_names, statements=statement_names, conditions=CONDITIONS, endpoints=ENDPOINTS, n_probe_fits=6 * len(splits), clean_probe_fits_reused=0, seed=SEED, bootstrap=args.bootstrap, sklearn_version=sklearn.__version__, numpy_version=np.__version__, sources_sha256={str(p): sha(p) for p in [args.representations, args.metadata, Path(__file__), *scripts]},
        protocol=f'Within-statement leave-one-actor-out: {2 * n_repetitions * (len(actor_names)-1)} train, {2 * n_repetitions} test. Train-only StandardScaler, C=1 lbfgs max_iter=5000, happy=1; all conditions independently fitted with identical splits.',
        primary='Block6 mean actor x statement fold AUC; direct paired contrasts no_attn-clean and no_attn-no_mlp. Equal actor and statement weights; repetitions pooled within each held-out actor/statement fold.',
        ...
        scope='Second-checkpoint replication on frozen existing datasets, fixed zero-based blocks1–6; no layer or condition reselection. This is not a cross-architecture claim. Within-statement probes do not establish classifier transfer to unseen statements.')
    (args.output_dir / 'analysis.json').write_text(json.dumps(info, indent=2) + '\n')
    plot(summary, args.output_dir / 'model_replication.png', args.dataset)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', choices=('ravdess', 'crema-d'), required=True)
