#!/usr/bin/env python3
"""Locate direct audio-to-decision mediation in individual and cumulative blocks."""
...
if __package__:
    from .run_audio_edge_restore import base, selected_attention, edge_output
else:
    from run_audio_edge_restore import base, selected_attention, edge_output
...
def restore_schedules():
    """Eleven unique schedules; cumulative 18 aliases singleton 18."""
    return {**{f'direct_block_{layer}': [layer] for layer in base.RESTORE_LAYERS},
            **{f'direct_cumulative_18_{end}': list(range(18, end + 1))
...
def run(args):
    ...
    groups = {'decision': [positions['decision_position']]}
    ...
    with args.output_csv.open('w', newline='') as stream:
        ...
        for index, row in enumerate(rows):
            ...
            def forward(audio=None, restore_layers=(), capture=False):
                handles = []
                current_cache = {}
                def capture_v(layer):
                ...
                def restore_edge(layer):
                    def hook(module, inputs):
                        ...
                        if not torch.isfinite(restored).all():
                            raise RuntimeError('Non-finite restored attention output')
                        others = sorted(set(range(original.shape[1])) - set(queries))
                        if others:
                            error = float((restored[:, others, :]-original[:, others, :]).abs().max())
                            if not math.isfinite(error):
                                raise RuntimeError('Non-finite nonselected query difference')
                            checks['nonselected_rows_max'] = max(checks['nonselected_rows_max'], error)
            ...
            reference = forward(audio=donor, restore_layers=base.RESTORE_LAYERS)
            ...
            for name, schedule in schedules.items():
                restored = forward(audio=donor, restore_layers=schedule)
                if index == 0:
                    checks['component_noop_max'] = max(checks['component_noop_max'], abs(forward(restore_layers=schedule)-clean))
                if schedule == base.RESTORE_LAYERS:
                    checks['joint_reference_max'] = max(checks['joint_reference_max'], abs(restored-reference))
                writer.writerow(dict(common, condition=name, component='audio_v_edge', restore_layers=json.dumps(schedule), clean_margin=clean, patched_margin=patched, restored_margin=restored, patch_effect=sign*(patched-clean), remaining_effect=sign*(restored-clean), mediated_effect=sign*(patched-restored), reference_source_margin=reference))
            stream.flush()
    ...
    meta = dict(n_samples=len(rows), n_pairs=len(pairs), seed=args.seed, positions=positions,
                ...
                numerical_checks=checks, tolerance=args.self_patch_tolerance, candidate_parity_samples=len(rows), joint_reference_samples=len(rows), noop_samples=1,
                upstream_patch='opposite emotion donor audio at post-block 17', restore_layers=base.RESTORE_LAYERS, schedules=schedules,
                schedule_aliases={'direct_cumulative_18': 'direct_block_18'},
                reference_source_margin_semantics='legacy column name: joint decision-query restore over blocks 18-23',
                ...
                dependency_sha256={name: hashlib.sha256(Path(__file__).with_name(name).read_bytes()).hexdigest()
                                   for name in ('run_audio_edge_restore.py', 'run_source_value_restore.py', 'run_residual_component_restore.py', 'run_activation_patching.py')})
